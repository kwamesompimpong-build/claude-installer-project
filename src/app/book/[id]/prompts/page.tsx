...
import { WRITING_PROMPTS, CATEGORY_LABELS, SUGGESTED_OUTLINE, getRandomPrompt } from "@/lib/prompts";
  ...
  return (
    <div className="min-h-screen">
      <header className="bg-white/80 border-b border-warm-200">
        <div className="max-w-5xl mx-auto px-6 py-4">
          ...
          <p className="text-warm-600 text-sm">
            Guided prompts to help you tell your caregiving story
          </p>
        </div>
      </header>
      ...
      <main className="max-w-5xl mx-auto px-6 py-8">
        {/* Spotlight prompt */}
        <div className="bg-sage-600 text-white rounded-xl p-6 mb-8">
          <p className="text-sage-200 text-xs uppercase tracking-wide mb-2">
            Writing Prompt
          </p>
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              className="bg-white text-sage-700 px-4 py-2 rounded-lg text-sm hover:bg-sage-50 transition-colors"
            >
              Start Writing This
            </button>
          </div>
          ...
          <div className="flex justify-between items-center mb-3">
            <div>
              ...
              <p className="text-warm-500 text-sm">
                A proven structure for caregiving memoirs — customize it to make
                it your own
              </p>
            </div>
            <button
            ...
            <div className="mt-4">
              <ol className="space-y-2">
                {SUGGESTED_OUTLINE.map((item, i) => (
                  <li
                    key={i}
                    className="flex items-center gap-3 text-warm-700 text-sm"
                  >
                    <span className="text-warm-400 font-mono text-xs w-6">
                      {i + 1}.
                    </span>
                    {item}
                  </li>
              ...
              <button
                ...
                className="mt-4 bg-sage-600 text-white px-5 py-2 rounded-lg hover:bg-sage-700 transition-colors text-sm"
              >
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {filteredPrompts.map((prompt) => (
            <div
              key={prompt.id}
              className="bg-white rounded-xl border border-warm-200 p-5 hover:shadow-sm transition-shadow"
            >
              <p className="text-xs text-sage-600 uppercase tracking-wide mb-2">
                {CATEGORY_LABELS[prompt.category]}
              </p>
              <p className="text-warm-800 font-serif leading-relaxed mb-3">
                {prompt.text}
              </p>
              <button
